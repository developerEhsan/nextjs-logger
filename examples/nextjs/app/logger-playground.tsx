...
const rapidLog = createLogger({
  namespace: "rapid-fire",
});
    ...
    <div className='flex flex-col gap-8 w-full max-w-2xl'>
      <section className='flex flex-col gap-3'>
        ...
        <p className='text-sm text-zinc-600 dark:text-zinc-400'>
          Each level uses its own TanStack Pacer strategy
          (throttle/debounce/rateLimit) before it reaches the terminal — see the
          README for the defaults.
        </p>
        <div className='flex flex-wrap gap-2'>
          {LEVELS.map((level) => (
            <button
              key={level}
              onClick={() =>
                log[level](`Manual ${level} log`, { clickedAt: Date.now() })
              }
      ...
      <section className='flex flex-col gap-3'>
        <h2 className='text-lg font-semibold'>
          2. Rapid-fire (throttling demo)
        </h2>
        <p className='text-sm text-zinc-600 dark:text-zinc-400'>
          Fires 20 debug logs synchronously from an isolated{" "}
          <code>createLogger</code> instance. Watch the terminal: TanStack Pacer
          throttles the relay, so you won&apos;t see 20 separate network calls
          land at once.
        </p>
        <button
          onClick={() => {
            for (let i = 0; i < 20; i++) {
              rapidLog.info(`Rapid entry ${i}`);
            }
          }}
          className='self-start rounded-full bg-foreground text-background px-4 py-2 text-sm cursor-pointer'
        >
          Fire 20 debug logs
        </button>
      </section>
      ...
      <section className='flex flex-col gap-3'>
        ...
        <p className='text-sm text-zinc-600 dark:text-zinc-400'>
          Logs an object containing a <code>password</code> field. Check the
          terminal — it prints <code>[REDACTED]</code>, never the real value.
        </p>
        <button
          onClick={async () => {
      ...
      <section className='flex flex-col gap-3'>
        ...
        <p className='text-sm text-zinc-600 dark:text-zinc-400'>
          The queue auto-flushes on a Pacer schedule and on tab close. This
          forces an immediate flush — useful right before a programmatic
          navigation.
        </p>
        <button
          onClick={async () => {
      ...
      <section className='flex flex-col gap-3'>
        ...
        <p className='text-sm text-zinc-600 dark:text-zinc-400'>
          Submits to a Server Action (<code>app/actions.ts</code>) using a
          namespaced{" "}
          <code>
            createLogger({"{"} namespace: &apos;auth&apos; {"}"})
          </code>
          . The password you type is logged server-side and redacted, exactly
          like #3.
        </p>
        <form
          action={async (formData) =>
            setLoginResult(await submitLogin(formData))
          }
        ...
        {loginResult && (
          <p
            className={
              loginResult.ok ? "text-sm text-green-600" : "text-sm text-red-600"
            }
          >
            {loginResult.message}
          </p>
      ...
      <section className='flex flex-col gap-3'>
        ...
        <p className='text-sm text-zinc-600 dark:text-zinc-400'>
          ...
          <code>runWithRequestContext()</code> — every log line it produces
          shares the same <code>requestId</code>, visible in the terminal
          output.
        </p>
        <button
          onClick={async () => {
            ...
            setOrdersResult(
              `requestId ${data.requestId} → ${data.orders.length} orders`,
            );
          }}
          className='self-start rounded-full border border-black/10 dark:border-white/20 px-4 py-2 text-sm hover:bg-black/5 dark:hover:bg-white/10'
        >
      ...
      <section className='flex flex-col gap-3'>
        ...
        <p className='text-sm text-zinc-600 dark:text-zinc-400'>
          ...
          <code>instrumentation.ts</code>&apos;s <code>onRequestError</code>{" "}
          hook logs it automatically — no try/catch anywhere in that route.
        </p>
        <button
          onClick={() => {
